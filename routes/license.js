...
router.post("/validate", async (req, res) => {
  try {
    ...
    console.log("Received license validation request:", {
      licenseKey,
      email,
      deviceId,
    });
    ...
    // Check key format
    const isFreeKeyFormat =
      /^[A-F0-9]{8}-[A-F0-9]{8}-[A-F0-9]{8}-[A-F0-9]{8}-[A-F0-9]{8}$/i.test(
        licenseKey
      );

    // Only allow your own key format
    if (!isFreeKeyFormat) {
      return res.json({
        valid: false,
        message: "Invalid license key format",
    ...
    if (!keyEntry) {
      return res.json({
        valid: false,
        message: "License key not found for this user",
      });
    }

    // Check if device is already registered
    if (keyEntry.registeredDevice) {
      if (keyEntry.registeredDevice === deviceId) {
        return res.json({
          ...
          maxDevices: 1,
          isDeviceRegistered: true,
          plan: keyEntry.variant_id || "free",
        });
      } else {
        return res.json({
          valid: false,
          message: "This license key is already registered to another device",
        });
      }
    }
